...
(function() {
  var camera, init, onResize, renderer, scene, tube;
  ...
  init = function() {
    var ambientLight, controls, cube, cubeGeometry, cubeMaterial, cylinder, cylinderGeometry, cylinderMaterial, gui, initStats, onDocumentMouseDown, onDocumentMouseMove, plane, planeGeometry, planeMaterial, projector, renderScene, scalingStep, sphere, sphereGeometry, sphereMaterial, spotLight, stats, step, webGLRenderer;
    // 场景
    ...
    onDocumentMouseDown = function(event) {
      ...
      if (intersects.length > 0) {
        ...
        return intersects[0].object.material.opacity = 0.1;
      }
    };
    onDocumentMouseMove = function(event) {
      var intersects, mat, points, raycaster, tubeGeometry, vector;
      if (controls.showRay) {
        vector = new THREE.Vector3((event.clientX / window.innerWidth) * 2 - 1, -(event.clientY / window.innerHeight) * 2 + 1, 0.5);
        vector = vector.unproject(camera);
        raycaster = new THREE.Raycaster(camera.position, vector.sub(camera.position).normalize());
